...
var recurFolder = function recurFolder(pathAbsFilOrFold) {
  var arrayString = [];
  ...
  if (fileOrFolder(pathAbsFilOrFold)) {
    ...
    var readFolder = fs.readdirSync(pathAbsFilOrFold);
    readFolder.forEach(function (nameFileOrFolder) {
      var newRoute = path.join(pathAbsFilOrFold, nameFileOrFolder);
